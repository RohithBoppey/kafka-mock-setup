...
# If failed, that message is pushed into this DLQ
try:
    dlq_producer = KafkaProducer(
        bootstrap_servers=[KAFKA_BROKER]
    )
    print("DLQ Producer connected.")
except Exception as e:
    print(f"Error connecting DLQ Producer: {e}")
    sys.exit(1)
...
try:
    consumer = KafkaConsumer(
        ...
        #### This is disabled because we want to CATCH ERRORS manually
...
def process_batch(batch):
    ...
    for i, message in enumerate(batch):
        ...
        try:
            # --- 1. TRY TO PROCESS ---
            # Step 1: Decode the bytes to a UTF-8 string
            value_str = message.value.decode('utf-8')
            
            # Step 2: Parse the string as JSON
            value_dict = json.loads(value_str)
            
            # Step 3: Access the data (This will fail on the "poison pill")
            ...
            # --- 3. FAILURE ---
            # Catches JSONDecodeError, TypeError, UnicodeDecodeError, etc.
            print("*" * 40)
            print(f"!!! FAILED TO PROCESS MESSAGE !!!")
            print(f"  Error:     {e}")
            print(f"  Offset:    {message.offset}")
            print(f"  Partition: {message.partition}")
            print(f"  Sending to DLQ topic: '{DLQ_TOPIC}'")
            
            # Send the *original, raw, failed* message to the DLQ
            dlq_producer.send(
                DLQ_TOPIC, 
                value=message.value,  # The original raw bytes
                key=message.key       # The original raw key
            )
            print("*" * 40)
            
            # NOTE: We do NOT commit here. The auto-commit will
            # eventually commit this offset, since we didn't crash.
